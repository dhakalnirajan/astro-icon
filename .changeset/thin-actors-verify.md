...
Widen the `astro` peer dependency from `^5.0.0` to `>=5.0.0 <8.0.0`.
...
Astro 8 doesn't have a published version yet (`npm view astro versions` shows no `8.x` releases as of this writing), so it's intentionally excluded from the range rather than guessed at.
